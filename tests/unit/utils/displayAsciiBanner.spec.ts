...
describe("displayAsciiBanner", () => {
  ...
  it("should log ASCII banner correctly", () => {
    ...
    assert(
      consoleLogStub.calledWithMatch("Manage Your Ascii Art"),
      "ASCII output should be logged",
    );
    assert(
      consoleLogStub.calledWithMatch("http://localhost:1234"),
      "Server link should be logged",
    );
  });
  ...
  it("should handle missing SERVICE_NAME gracefully", () => {
    ...
    assert(
      consoleErrorStub.calledWithMatch("❌ No ASCII art data generated"),
      "Error should be logged",
    );
    assert(
      consoleLogStub.notCalled,
      "console.log should not be called if no ASCII art is generated",
    );
  });
  ...
  it("should handle figlet error", () => {
    ...
    assert(
      consoleErrorStub.calledWithMatch("❌ Error generating ASCII art:"),
      "Error should be logged on figlet failure",
    );
  });
});
