  ...
  try {
    ...
    routeFiles.forEach((filePath) => {
      if (fs.existsSync(filePath)) {
        ...
        const mountMatches = content.match(
          /(router|app)\.use\(['"`]([^'"`]+)['"`]/g,
        );
        if (mountMatches) {
    ...
    if (fs.existsSync(routesDir)) {
      ...
      routeFiles.forEach((file) => {
        if (file.endsWith(".ts") || file.endsWith(".js")) {
          ...
          const routeMatches = routeContent.match(
    ...
    console.error(
      "Route detection may be incomplete. Please check the codebase scanning logic.",
    );
      ...
      subLayer.handle.stack.forEach((routeLayer) => {
        if (routeLayer.route) {
          ...
          methods.forEach((method) => {
            if (method !== "_all") {
              const fullPath =
                mountPath === "/" ? route.path : mountPath + route.path;
              routes.push({
