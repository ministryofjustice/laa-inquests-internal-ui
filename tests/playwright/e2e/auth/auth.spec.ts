...
test("redirects unauthenticated user to Entra on GET /auth/login", async ({ page }) => {
  const response = await page.goto("/auth/login", { waitUntil: "commit" });

  expect(response?.url()).toContain(AUTH_AUTHORITY_URL);
});

test("renders home page when authenticated and session has UserId", async ({ page }) => {
...
test("logout clears the session and redirects to post-logout URI", async ({ page }) => {
